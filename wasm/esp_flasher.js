...
function __asyncjs__js_serial_reset_target(reset_hold_ms) { return Asyncify.handleAsync(async () => { const transport = Module.__transport; if (!transport || typeof transport.setSignals !== "function") { console.error("[ERROR] Transport not registered on Module.__transport"); return; } try { await transport.setSignals({ dataTerminalReady: true, requestToSend: true }); await new Promise((resolve) => setTimeout(resolve, reset_hold_ms)); await transport.setSignals({ dataTerminalReady: true, requestToSend: false }); } catch (error) { console.error("[ERROR] Failed to reset target:", error); } }); }
