...
var ASM_CONSTS = {
  165637: ($0) => { Module._timerEnd = Date.now() + $0; },  
 165677: () => { if (typeof Module._timerEnd === "undefined") return 0; const r = Module._timerEnd - Date.now(); return r > 0 ? r : 0; }
};
...
function __asyncjs__js_serial_read(data,size,timeout_ms) { return Asyncify.handleAsync(async () => { try { if (typeof Module.serialBuffer === "undefined") { Module.serialBuffer = new Uint8Array(0); } if (timeout_ms === 0) { return -2; } const deadline = Date.now() + timeout_ms; while (Module.serialBuffer.length < size) { if (Date.now() >= deadline) { return -2; } await new Promise((resolve) => setTimeout(resolve, 0)); } for (let i = 0; i < size; i++) { HEAPU8[data + i] = Module.serialBuffer[i]; } Module.serialBuffer = Module.serialBuffer.slice(size); return 0; } catch (error) { console.error("[ERROR] Serial read failed:", error); return -1; } }); }
